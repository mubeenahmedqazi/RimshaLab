...
export async function getUnreadMessages() {
  try {
    const res = await fetch(`${API_BASE_URL}/contact?status=unread`, { cache: "no-store" });
    return await res.json();
  } catch (error) {
...
export async function getReadMessages() {
  try {
    const res = await fetch(`${API_BASE_URL}/contact?status=read`, { cache: "no-store" });
    return await res.json();
  } catch (error) {
...
export async function getChartStats() {
  try {
    const [bookingsRes, patientsRes, healthCardsRes] = await Promise.all([
      fetch(`${API_BASE_URL}/bookings`, { cache: "no-store" }),
      fetch(`${API_BASE_URL}/health-card`, { cache: "no-store" }),
      fetch(`${API_BASE_URL}/health-card`, { cache: "no-store" }), // same endpoint but we can filter if needed
    ]);
    ...
    const patientsByDay = groupByDay(patientsData.healthCards || []); // if patients tracked via health card
    const healthCardsByDay = groupByDay(healthCardsData.healthCards || []);

    const finalData = Object.keys(bookingsByDay).map((day) => ({
      name: day,
      bookings: bookingsByDay[day] || 0,
      patients: patientsByDay[day] || 0,
      healthCards: healthCardsByDay[day] || 0, // ✅ include this
    }));

    return finalData;
  } catch (error) {
    console.log("Error fetching chart stats:", error);
    return [];
  }
}
